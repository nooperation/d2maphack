...
char *szProgTitle = "Sting's Diablo II Hackmap v1.10 (v1.15)";
void KeepGameWindow();
...
{
	if (!D2CLIENT_nQuestPage) {
		int cow = D2COMMON_GetQuestFlag(D2CLIENT_GetQuestInfo(), 4, 10);
		if (cow) D2DrawCenterText(8, L"X", 307+xpos, 48-ypos, cow?1:2);
	}
}
...
void __fastcall GameTimePatch(DWORD size)
{
	...
	if (tToggleGameTime.flag) {
		...
		D2DrawCenterText( 8, buf, D2GetScreenSizeX()-16, ypos, 4, 0);
		ypos += 16;
	}
...
struct PlayerStat {
	...
	{STAT_MAGIC_FIND, "Magic Find"},
	{STAT_EXTRA_GOLD, "Extra Gold"},
	{STAT_IAS, "Increased Attack Speed"},
	{STAT_FCR, "Faster Cast Rate"},
	{STAT_FBR, "Faster Block Rate"},
	{STAT_FHR, "Faster Hit Recovery"},
	{STAT_FRW, "Faster Run/Walk"},
	{STAT_LIFE_LEECH, "Life Leech"},
	{STAT_MANA_LEECH, "Mana Leech"},
	{STAT_DAMAGE_REDUCED, "Damage Reduced"},
	{STAT_MAGIC_DAMAGE_REDUCED, "Magic Damage Reduced"},
	{STAT_LIGHTING_ABSORB, "Lighting Absorb"},
	{STAT_FIRE_ABSORB, "Fire Absorb"},
	{STAT_COLD_ABSORB, "Cold Absorb"},
	{STAT_MANA_AFTER_EACH_KILL, "Mana After Each Kill"},
	{STAT_DAMAGE_TAKEN_GOES_TO_MANA, "Damage Taken Goes to Mana"},
//	{STAT_MAGIC_ABSORB, "Magic Absorb"},
//	{STAT_MAGIC_RESIST, "Magic Resist"},
//	{STAT_DEADLY_STRIKE, "Deadly Strike"},
//	{STAT_PIERCING_ATTACK, "Piercing Attack"},
//	{STAT_BLESSED_AIM, "Blessed Aim"},
//	{STAT_REGENERATE_MANA, "Regenerate Mana"}
};
...
void ViewPlayerStatPatch()
{
	...
	int pla = GetPlayerStat(unitpla);
	UnitAny *unit = D2CLIENT_GetUnitFromId(D2CLIENT_GetInventoryId(unitpla, 7, 0), 1);
	int pet = GetPlayerStat(unit);

	wsprintfW2(temp, "%s", aPlayerStats[nCurrentPlayerStat].desc);
	D2DrawText( 6, temp, D2GetScreenOffsX()+16, D2GetScreenOffsY()+380, 0);
	if (aPlayerStats[nCurrentPlayerStat].stat != STAT_MAGIC_DAMAGE_REDUCED)
		wsprintfW2(temp, "%d%% %d%%", pla, pet);
	else
		wsprintfW2(temp, "%d %d", pla, pet);
	D2DrawText( 6, temp, D2GetScreenOffsX()+16, D2GetScreenOffsY()+380+12, 0);
}
	...
	if (len >= 2) {
		...
		for (i=0, j = 0; i < len; i++) {
			...
			if (!HIBYTE(char1) && (char1) & 0x80 && char1 != 0xff && LOBYTE(char2)) {
				...
				if (MultiByteToWideChar(CP_ACP, 0, (char *)mbchar, sizeof(mbchar), &uchar, 1)) {
					lpText[j++] = uchar;
					i++;
					if (i == dwOldPos) {
						dwNewPos = j-1;
...
HANDLE hD2LocaleMPQ;
void FontFilePatch(LPSTR lpText)
{
	if (!hD2LocaleMPQ) return;
...
DWORD __fastcall D2LocaleChannelChat(LPWSTR lpText, DWORD u1, DWORD u2,DWORD u3, DWORD u4, DWORD u5)
{
	D2UnicodeFix(lpText, NULL);
	return D2WIN_Locale_Func(lpText, u1, u2, u3, u4, u5);
}

DWORD __fastcall D2Win_Patch2(DWORD u1, LPSTR lpText, DWORD u2, LPCSTR file, DWORD line)
{
	FontFilePatch(lpText);
	return D2WIN_Patch2_Func(u1, lpText, u2, file, line);
}

BYTE fInstallPatchs2;
DWORD __fastcall D2Win_Patch3(DWORD u1, LPSTR lpText, DWORD dwSize)
{
	if(!fInstallPatchs2) {
		InstallPatchs2();
	}
	...
	DWORD ret;
	ret = D2WIN_Patch3_Func(u1, lpText, dwSize);
	FontFilePatch(lpText);
	return ret;
}
...
DWORD __fastcall ChannelEnterCharPatch(D2EditBox *hWnd, BYTE bKeyCode)
{
	...
	if (bKeyCode >= ' ') {
		...
		*/
		...
		wcscpy(lpText, lpTmp);
		/*
		if (WideCharToMultiByte(CP_ACP, 0, lpText, len+1, lpTmp, (len+1)*2, NULL, NULL)) {
		len = strlen(lpTmp);
...
void __stdcall D2MultiByteFix(LPSTR str)
{
	...
	tagChat *p = (tagChat*)str;

	char *p2 = p->txt + strlen(p->txt) + 1;
	for (LPCSTR p = p2; *p; ++p) {
		if ((BYTE)*p == 0xc3) {
			*p2++ = *(++p)+0x40;
		} else if ((BYTE)*p == 0xc2) {
...
void __stdcall RButtonUpHandlerPatch(D2MSG *pmsg)
{
	if (tToggleRightClickSwap.flag) {
		...
		if (D2CLIENT_GetUiVar(UIVAR_STASH))
			...
			loctype = 2;
		if (pmsg->message == WM_RBUTTONUP && loctype) {
			UnitAny *pla = D2CLIENT_GetPlayerUnit();
...
void DrawMinimapLevel(int lvlno)
{
	...
	if( cit != anMinimapCells.end() )
	{
		...
		for( i = 0; i < c; i++ )
		{
			...
			DrawMinimapCell(xpos, ypos, nMinimapCellCol);
		}
	}
}
char *szLocaleMPQ;
...
#else
	if (chksum != 0x4c91d895) return FALSE;
#endif
