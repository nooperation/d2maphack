...
setmefree:
		// free myself
		// pereare code for after VirtualFree return
...
{
	ASSERT(pid && lpszModuleName);
	CModuleWalker walker;
	if( !walker.Walk(pid) ) return FALSE;
...
LPVOID CRemoteExecute::VirtualAllocEx9x(DWORD nSize)
{
	...
	if (!PatchAddress) { 
		GlobalFormatAndReportStatus(IDS_ERR_CANNT_FIND, _T("a patch location"));
		return NULL;
	}
	PatchAddress += 0x400;
...
BOOL CRemoteExecute::Execute(entry_t entry, DWORD nEntrySize, LPVOID param, DWORD nParamSize, DWORD& result)
{
	...
	if (!WinProcAddress)
	{
		GlobalFormatAndReportStatus(IDS_ERR_CANNT_FIND, _T("WinProc entrypoint"));
		return FALSE;
	}
	...
	*(FARPROC*)&context.fnGetLastError = GetProcAddress(hKernel32, "GetLastError");
	if(!context.fnVirtualProtect || !context.fnWriteProcessMemory || !context.fnCloseHandle
		|| !context.fnGetLastError) {
			GlobalFormatAndReportStatus(IDS_ERR_CANNT_FIND, _T("Win32 API needed for remote routine"));
			return FALSE;
		}
